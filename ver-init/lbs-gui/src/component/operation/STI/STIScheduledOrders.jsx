...
export default () => {
    ...
    // table
    const { STIData, setSTIData } = useGlobalContext();
    const columns = [
        {
            title: '工作單編號', dataIndex: 'order_No', key: 'order_No', width: 120,
            ...getColumnSearchProps('order_No', '工作單編號', searchInput, setSearchText, searchText, setSearchedColumn, searchedColumn),
        },
        {
        ...
        },
        {
            ...
            ...getColumnSearchProps('date', '日期', searchInput, setSearchText, searchText, setSearchedColumn, searchedColumn),
        },
        {
        ...
        {
            title: '客戶名稱', dataIndex: 'customerName', key: 'customerName', width: 120, ellipsis: { showTitle: false, },
            ...getColumnSearchProps('customerName', '客戶名稱', searchInput, setSearchText, searchText, setSearchedColumn, searchedColumn),
        },
        {
    ...
    const handleDelete = (key) => {
        const newData = STIData.filter((item) => item.key !== key);
        setSTIData(newData);
    };
    ...
    const handleImport = useCallback((e) => {
        ...
        if (files) {
            ...
            readXlsxToJson(f, (result) => {
                setSTILoading(false);
                setSTIData(convertJsonData(result, columns));
            });
        }
    }, []);
    ...
    return (
        <div>
            <FlexFileInput text="導入全部工作單" accept=".xlsx,.xls" onChange={handleImport} />
